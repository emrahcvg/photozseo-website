...
export async function handleMarket(parts: string[], deps: MarketDeps): Promise<Response> {
  ...
  if (parts.length === 0) {
    ...
    return htmlResponse(renderDocument({
      ...
      bodyScripts: ['https://accounts.google.com/gsi/client', '/auth.js?v=8', '/marketplace-enhance.js?v=8'],
    }));
  }
  ...
  if (parts[0] === 'favorites' && parts.length === 1) {
    ...
    return htmlResponse(renderDocument({
      ...
      stylesheets: ['/marketplace.css?v=8'],
      bodyScripts: ['https://accounts.google.com/gsi/client', '/auth.js?v=8', '/marketplace-enhance.js?v=8', '/market-account.js?v=8'],
    }));
  }
