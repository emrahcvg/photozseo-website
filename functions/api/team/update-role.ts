/**
 * POST /api/team/update-role — bir üyenin rolünü değiştirir (employee↔admin).
 * Yalnızca manageMembers yetkisi olan rol (owner/admin). Kurallar:
 *  - kendi rolünü değiştiremez
 *  - owner'ın rolü değiştirilemez (devir ayrı flow)
 *  - yeni rol yalnız 'admin' | 'employee' (owner ataması = devir, burada değil)
 *  - admin başka bir admin'in rolüne dokunamaz (remove-member ile tutarlı)
 * Gövde: JSON { companyId, sub, role }
...
export async function onRequestPost(ctx: Ctx): Promise<Response> {
  ...
  // Yeni rol yalnız admin|employee olabilir; owner ataması bu endpoint'te yapılmaz (devir ayrı).
  ...
  // Owner sabit; admin başka admin'e dokunamaz.
