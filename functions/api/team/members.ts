/**
 * GET /api/team/members?companyId=X — bir şirketin üye listesini (roster) döner.
 * Çağıran o şirkette üye olmalı (Owner/Admin/Employee hepsi salt-okunur görebilir).
 * 200 { ok, members: [{ sub, email, name, role, joinedAt }] } | 400 | 401 | 403 | 503
 */
import { resolveMember } from '../../_lib/team-session';
import { getMembership, listCompanyMembers } from '../../_lib/team';
...
export async function onRequestGet(ctx: Ctx): Promise<Response> {
  ...
  const members = await listCompanyMembers(ctx.env.MARKET_DB, companyId);
