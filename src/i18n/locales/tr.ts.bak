import type { HomeContent } from '../home';

const tr: HomeContent = {
  nav: {
    features: 'Özellikler', pricing: 'Fiyatlandırma', faq: 'SSS',
    blog: 'Blog', support: 'Destek', getApp: 'Uygulamayı İndir',
  },
  hero: {
    eyebrow: 'Artık iPhone · iPad · Mac üzerinde',
    h1Line1Words: ['Bir', 'kez', 'çek.'],
    h1Line2Words: ['46', 'pazaryerinde', 'sat.'],
    h1A11y: 'Bir kez çek. 46 pazaryerinde sat.',
    sub: '7-açılı AI kamera, 12 dilde cihaz üstünde SEO, 4K\'ya kadar 31 boyut ön ayarı ve platforma kusursuz dışa aktarımlar — hepsi iPhone, iPad veya Mac\'inden.',
    btnAppSmall: 'Erken',
    btnAppBig: 'Erişim',
    ctaTrial: 'Erken Erişim Al',
    ctaCard: 'Erken erişim · Kart gerekmez',
    bullets: [
      '7-açılı şablon kamera — Önden, Arkadan, Yandan, Detay, Üstten, Etiket, Lifestyle',
      'Yapay zekâ tek fotoğraftan 12 dilde 13 SEO alanını dolduruyor',
      '46 pazaryeri, tek dokunuş · %100 cihaz üstünde · çevrimdışı çalışır',
    ],
  },
  pain: {
    kicker: 'Neden var?',
    title: 'Her pazaryerinde satış yapmak bozuk bir sistem.',
    sub: 'Sen bir satıcısın, Photoshop teknisyeni değil. photoZseo o saatleri sana geri veriyor.',
    cards: [
      {
        bad: '"Her platform için fotoğraf düzenlemek bütün akşamımı alıyor."',
        goodTitle: 'Yapay zekâ 2 saniyede yapıyor.',
        goodDesc: 'Saliency kırpma, beyaz dengesi, 31 dışa aktarım boyutu — her platform için otomatik.',
      },
      {
        bad: '"Her pazaryeri için başlığı, etiketleri ve açıklamayı tekrar tekrar yazıyorum."',
        goodTitle: 'Tek fotoğraf. 13 SEO alanı. Dolduruldu.',
        goodDesc: 'Başlık, slug, meta, alt metin, anahtar kelimeler — cihaz üstünde çalışan yapay zekâ, 12 dilde yazıyor.',
      },
      {
        bad: '"B2B tekliflerim utanç verici bir Word belgesi gibi duruyor."',
        goodTitle: 'Markalı PDF\'ler 3 adımda.',
        goodDesc: 'VAT, IBAN, HS kodları, e-imza, tutar yazıyla — Fortune 500 faturası gibi görünüyor.',
      },
    ],
  },
  features: {
    sectionKicker: 'Tüm özellikler, tek bir kaydırmada',
    sectionTitle: 'On özellik. Tek bir kaydırma.',
    sectionSub: 'Pazaryerleri, 7-açılı AI kamera, SEO otomatik doldurma, toplu içe aktarım, B2B teklifler, iCloud senkronu ve dahası — her gün gönderim yapan bağımsız satıcılar için.',
    seeInAction: 'İş başında gör →',
    items: [
      {
        eyebrow: '46 Pazaryeri',
        h: 'Bir kez listele. Her yerde sat.',
        sub: 'Tek bir fotoğraf, 46 pazaryerine hazır listeye dönüşür — her platform için boyutlandırılmış, isimlendirilmiş ve SEO etiketli.',
        bullets: [
          'Amazon · Shopify · Etsy · Walmart · eBay · Trendyol',
          'Sınır ötesi: Temu · SHEIN · DHgate · Made-in-China · Global Sources',
          'CSV / XLSX / JSON — ürün başına ayrı veya birleşik',
        ],
      },
      {
        eyebrow: 'AI Kamera · 7 Açı, Tek Deklanşör',
        h: 'Önden, Arkadan, Yandan, Detay, Üstten, Etiket, Lifestyle.',
        sub: 'Her pazaryerinin istediği 7 açıyı sana adım adım rehberlik eden şablon tabanlı bir kamera. Saliency tabanlı otomatik yakınlaştırma (4×), HDR, anti-flicker ve 12 dilde canlı OCR — her kare aynı anda JPEG, PNG ve WebP olarak dışa aktarılır.',
        bullets: [
          '7-açılı şablonlar ile otomatik yön ilerletme',
          'Akıllı otomatik yakınlaştırma (%25 özne doluluğu, max 4×) · AE Lock · WB Auto',
          'Cihaz üstünde Vision: saliency, sınıflandırma, barkod, canlı OCR',
          'Ses tuşu deklanşörü · 4K çekim · %100 çevrimdışı',
        ],
      },
      {
        eyebrow: 'AI SEO Otomatik Doldurma',
        h: '13 alanı saniyeler içinde doldur.',
        sub: 'Bir fotoğraf at. photoZseo başlığı, slug\'ı, kategoriyi, meta\'yı, tam açıklamayı, alt metni, etiketleri, anahtar kelimeleri ve barkodu — arama için optimize ederek 12 dilde anında yazar.',
        bullets: [
          'Cihaz üstünde OCR + Vision: barkod, marka, malzeme, renk',
          'SEO seviyesinde başlık, slug, kategori, meta, uzun açıklama, alt metin ve uzun kuyruklu anahtar kelimeler',
          '12 dil — yerel kurallara uygun çıktılar, RTL farkında',
        ],
      },
      {
        eyebrow: 'B2B Teklif Oluşturucu',
        h: '3 adımda markalı PDF teklifler.',
        sub: 'Ürünleri seç, alıcıyı ekle, paylaş. VAT, IBAN, Incoterms, HS kodları ve e-imza ile 12 dilde A4 PDF\'ler — şık ve markalı.',
        bullets: [
          'Satır başına VAT · birim · HS kodu · menşe ülke',
          'Logo, IBAN, Incoterms ile gönderici profili — bir kez kaydet',
          'Kartvizit tarama (Vision OCR) alıcı bilgilerini otomatik doldurur',
        ],
      },
      {
        eyebrow: 'iCloud Senkronu ve Çöp Kurtarma',
        h: 'Her yerde senkron. Her şeyi geri yükle.',
        sub: 'Her proje kendi iCloud Drive\'ından senkronlanır. Silinen öğeler kalıcı temizlikten önce 30 gün bekler; iPhone, iPad ve Mac arasında güvenli geri yükleme.',
        bullets: [
          'iCloud Drive senkronu — opsiyonel, son-yazan-kazanır',
          'Son Silinenler klasörü ile güvenli geri yükleme',
          '30 gün otomatik temizleme süresi (7 / 14 / 30 / 60 gün ön ayarları)',
          'Cihazlar arası kalıcı silme propagasyonu',
        ],
      },
      {
        eyebrow: 'Ürün Detayı',
        h: 'Profesyonel gibi yönet.',
        sub: 'Bir ürün, her kanal. Fotoğraflar, fiyatlar, SEO, EXIF ve story\'ler tek bir özenle düzenlenmiş kayıtta yaşar — B2B için 13-renk kişi paleti dahil.',
        bullets: [
          'Fotoğraf karuseli, medya denetçisi, EXIF satır içi düzenleyici',
          'Ürün başına markup, MSRP, B2B fiyat, VAT modu',
          'Teklif, Katalog veya Pazaryerine hızlı paylaş',
        ],
      },
      {
        eyebrow: 'Toplu CSV / XLSX İçe Aktarma',
        h: 'Katalog at. 41 platforma eşle.',
        sub: 'Bir katalog dosyası at — photoZseo 41 platform imzasını otomatik tanır ve ürün fotoğraflarını paralel olarak çeker. Mevcut Shopify veya Amazon katalogların dakikalar içinde canlı.',
        bullets: [
          'CSV · TSV · XLSX · JSON katalog içe aktarımı',
          'Başlıklardan 41 platform imzası otomatik tespit',
          'Fotoğraf URL\'leri paralel indirme · Wi-Fi only seçeneği',
          'Platform başına toplu ZIP dışa aktarım',
        ],
      },
      {
        eyebrow: 'Mac + iPhone + iPad',
        h: 'Yerel Mac uygulaması. Evrensel binary.',
        sub: 'iOS 17+, iPadOS ve Mac Catalyst ile macOS 14+. Klavye kısayolları, Finder sürükle-bırak, 3-panel denetçi ve iCloud-senkronlu projeler — her cihazda.',
        bullets: [
          '⌘N / ⌘E / ⌘F / ⌘A klavye kısayolları',
          'Finder\'dan sürükle-bırak · sağ tık bağlam menüleri',
          '3-panel denetçi: kenar çubuğu · grid · detay',
          'iPhone, iPad ve Mac arasında iCloud Drive senkronu',
        ],
      },
      {
        eyebrow: 'Önce Gizlilik',
        h: 'Takip yok. Reklam yok. Bulut yok.',
        sub: 'Buluta güvenmeyen satıcılar için yapıldı. GDPR, KVKK ve CCPA uyumlu. 3 RTL alfabesi dahil 12 dil. Dışa aktarımda GPS varsayılan olarak silinir.',
        bullets: [
          'GDPR · KVKK · CCPA — analiz yok, üçüncü taraf SDK yok',
          'EN · TR · DE · ES · FR · PT · JA · KO · ZH · AR · HI · FA',
          'Yapay zekâ cihaz üstünde çalışır — fotoğrafların asla cihazını terk etmez',
          'Dışa aktarımda GPS verisi silinir · yazılım etiketi korunur',
        ],
      },
      {
        eyebrow: 'Fiyatlandırma',
        h: 'Başlangıçta ücretsiz. Hazır olunca Pro.',
        sub: 'Ücretsiz başla. Sadece her pazaryerine dışa aktarmaya hazır olduğunda yükselt. İstediğin zaman iptal, erken erişim için kart gerekmez.',
        bullets: [
          'Çekim, organizasyon ve temel dışa aktarım sonsuza dek ücretsiz',
          'Pro 46 pazaryeri formatı, AI SEO, B2B teklif, toplu içe aktarım ekler',
          'Haftalık $2.99 · Aylık $6.99 · Yıllık $39.99 (en iyi değer)',
        ],
      },
    ],
  },
  bento: {
    headKicker: 'Sadece ekran görüntülerinden ibaret değil',
    headTitle: 'Bir güç aleti gibi yapıldı, bir uygulama gibi fiyatlandırıldı.',
    headSub: 'Aşağıdaki her bölüm bugün hazır olan bir özelliktir — yol haritası yok, bekleme listesi yok.',
    csv: {
      label: 'CSV · XLSX · JSON',
      sub: '46 pazaryeri formatı — başlığı bir kez yaz, tüm sütunlar dolsun.',
    },
    exif: {
      label: 'Kamera Seviyesinde EXIF',
      gpsStripped: '— TEMİZLENDİ —',
    },
    langs: {
      label: '12 Dil · 3 RTL',
      sublabel: 'EN · TR · DE · ES · FR · PT · JA · KO · ZH · AR · HI · FA',
    },
    stats: {
      label: 'Bağımsız satıcılar için yapıldı',
      marketplaces: 'pazaryeri',
      seoFields: 'fotoğraf başına SEO alanı',
      devices: 'cihaz · tek uygulama',
      noTrackers: 'tracker · reklam · bulut',
    },
  },
  seoDemo: {
    kicker: 'Etkileşimli · Canlı',
    title: 'AI\'ın 13 SEO alanını nasıl doldurduğunu izle.',
    sub: 'Bir ürün seç. photoZseo\'nun pazaryerinin ihtiyacı olan her alanı — başlık, slug, kategori, meta, tam açıklama, alt metin, etiketler, anahtar kelimeler ve barkod — nasıl yazdığını izle. Kopyalamak için herhangi bir alana dokun.',
    inputLabel: 'Ürün adı',
    inputPlaceholder: 'Bir ürün adı yaz…',
    btnIdle: '✨ 13 SEO alanını otomatik doldur',
    btnBusy: 'Oluşturuluyor…',
    note: 'Cihaz üstünde · %100 gizli · 12 dil',
    fieldLabels: {
      title: 'SEO Başlığı',
      slug: 'Slug',
      filename: 'Dosya Adı (URL)',
      category: 'Kategori',
      meta: 'Meta Açıklama',
      description: 'Açıklama',
      alt: 'Alt Metin',
      tag1: 'Etiket 1',
      tag2: 'Etiket 2',
      tag3: 'Etiket 3',
      tag4: 'Etiket 4',
      keywords: 'Anahtar Kelimeler',
      barcode: 'Barkod',
    },
  },
  stats: {
    kicker: 'Ürünü hareket ettiren rakamlar',
    title: 'Bağımsız bir satıcı tarafından tasarlandı.\n12 ülkedeki alıcılar tarafından test edildi.',
    labels: {
      marketplaces: 'Pazaryerleri ve kanallar',
      seoFields: 'Fotoğraf başına SEO alanı',
      languages: 'Dil — 3 RTL',
      zero: 'Reklam. Tracker. Toplanan veri.',
    },
    pills: {
      gdpr: 'GDPR', kvkk: 'KVKK', ccpa: 'CCPA',
      wcag: 'WCAG 2.1 AA', noTracking: 'Asla takip yok',
    },
  },
  pricing: {
    kicker: 'Fiyatlandırma',
    title: 'Daha hızlı sat — filigran yok, sınır yok.',
    sub: 'Üç plan, tek Pro deneyim. Yıllık plan en iyi değer: ayda yalnızca $3.33 — istediğin zaman Ayarlar\'dan iptal et.',
    badge: 'En iyi değer · %52 tasarruf',
    plans: [
      {
        name: 'Haftalık',
        pricePer: '$2.99/hafta',
        features: [
          'Tam Pro erişim',
          'Bir hafta boyunca dene',
          'Ayarlar\'dan istediğin zaman iptal',
          '— Uzun vadeli taahhüt yok',
        ],
        cta: 'Haftalığı Başlat',
      },
      {
        name: 'Aylık',
        pricePer: '$6.99/ay',
        features: [
          'Tam Pro erişim',
          'Aylık faturalı, istediğin zaman iptal',
          'Yıllık ile aynı özellikler',
          '— Yıllık plan ile %52 tasarruf',
        ],
        cta: 'Aylığı Seç',
      },
      {
        name: 'Yıllık',
        pricePer: '$39.99/yıl',
        trial: 'Ayda yalnızca $3.33 · %52 tasarruf',
        featured: true,
        features: [
          '**Filigranı kaldır** tüm dışa aktarımlarda',
          '**Toplu dışa aktar** her fotoğrafı tek seferde',
          '**CSV + XLSX** 46 pazaryeri için',
          '**7-açılı şablon kamera** + Pro Araçlar',
          'AI SEO otomatik doldurma (13 alan, 12 dil)',
          'B2B teklif oluşturucu (VAT, IBAN, HS, e-imza)',
          'Toplu CSV / XLSX / JSON katalog içe aktarımı',
          'EXIF düzenleyici ve metadata pipeline (GPS silinir)',
          'iPhone, iPad ve Mac evrensel',
          'iCloud Drive senkronu + Çöp kurtarma',
        ],
        cta: 'Yıllığı Al',
      },
    ],
    note: 'Fiyatlar USD cinsindendir. App Store\'da yerel fiyatlandırma uygulanır (örn. Türkiye\'de ₺39.99/yıl). Aile Paylaşımı desteklenir. Abonelikler dönem bitiminden en az 24 saat önce iptal edilmediğinde otomatik yenilenir — Ayarlar → Apple ID → Abonelikler bölümünden yönet.',
  },
  faq: {
    kicker: 'Sorular',
    title: 'Sormak istediğin her şey.',
    items: [
      {
        q: 'photoZseo internet olmadan çalışır mı?',
        a: 'Evet. Fotoğraf çekimi, düzenleme, AI SEO otomatik doldurma, EXIF ve dışa aktarımların tümü Apple Foundation Models ve Vision kullanılarak %100 cihaz üstünde çalışır. İnternet sadece uzaktaki bir pazaryerine aktif olarak gönderim yaparken gereklidir.',
      },
      {
        q: 'Aynı anda Amazon, Shopify ve Etsy\'ye dışa aktarabilir miyim?',
        a: 'Evet. Ürünleri seç, platformları seç, Dışa Aktar\'a bas. photoZseo, her platformun tam sütun sırasında CSV/XLSX/JSON yazar, ayrıca platform başına resim boyutları — ürün başına ayrı veya tek bir birleşik dosya olarak.',
      },
      {
        q: 'Fotoğraflarım veya ürün verilerim herhangi bir yere yükleniyor mu?',
        a: 'Hayır. Sıfır üçüncü taraf analiz, sıfır takip SDK\'sı ve bulut hesabı yok. Tek dış ağ bağlantısı abonelik için App Store / RevenueCat ve seçeneğe bağlı, kendi iCloud\'unda kalan iCloud Drive senkronudur.',
      },
      {
        q: 'iPad ve Mac\'te çalışır mı yoksa sadece iPhone mu?',
        a: 'Üçünde de. photoZseo evrensel bir uygulamadır — aynı satın alma iPhone, iPad ve Mac (Catalyst, macOS 14+) kapsar. Finder\'dan sürükle-bırak, ⌘N/E/F/A klavye kısayolları, sağ tık menüleri ve 3-panel denetçi iPad ve Mac\'te birinci sınıftır.',
      },
      {
        q: 'Pro\'da tam olarak ne var?',
        a: 'Her şey: 46 pazaryeri formatı, AI SEO otomatik doldurma (13 alan, 12 dil), B2B teklif oluşturucu, 7-açılı şablon kamera, toplu CSV/XLSX içe aktarım, filigran kaldırma ve Çöp kurtarmalı iCloud senkronu.',
      },
      {
        q: '"46" tam olarak hangi platformlar?',
        a: 'Lansmanda: 46 pazaryeri ön ayarı (Amazon, Shopify, Etsy, eBay, Walmart, WooCommerce, Trendyol, AliExpress, Mercado Libre, Coupang, Lazada, Shopee, Temu, SHEIN, DHgate, Made-in-China, Global Sources…) artı sosyal/mesajlaşma formatları (Instagram Shopping, TikTok Shop, Pinterest, WeChat, RED, Stories). Yeni ön ayarlar güncellemelerle gelir.',
      },
      {
        q: 'GPS konumum dışa aktarılan fotoğraflarda olacak mı?',
        a: 'Hayır. photoZseo dışa aktarımda GPS verisini varsayılan olarak siler. Diğer EXIF — Artist, Copyright, Software, boyutlar, kamera — korunur ve satır içi düzenlenebilir. Kontrol sende.',
      },
      {
        q: 'GDPR / KVKK / CCPA uyumlu mu?',
        a: 'Evet — ve uygulama senin dilini konuşuyor: İngilizce, Türkçe, Almanca, İspanyolca, Fransızca, Portekizce, Japonca, Korece, Çince, Arapça, Hintçe ve Farsça. Arapça, Farsça ve Urduca için RTL düzenleri birinci sınıftır.',
      },
      {
        q: 'Mevcut ürün listemi içe aktarabilir miyim?',
        a: 'Evet. CSV, TSV, XLSX, JSON ve fotoğraf URL\'leri desteklenir. Dosya başlıklarından otomatik tespit ile 41 platform içe aktarma şablonu dahildir — mevcut bir Shopify veya Amazon dışa aktarımını at, gerisini photoZseo halleder.',
      },
    ],
  },
  finalCta: {
    h2: 'Bir sonraki satışın tek bir fotoğrafla başlıyor.',
    p: 'Başlangıçta ücretsiz. Takip yok. Reklam yok. İstediğin zaman iptal. Bağımsız satıcılar için yapıldı.',
    note: 'Yakında App Store\'da.',
    notify: 'Lansmanda haber ver',
  },
  footer: {
    tag: 'Tek bir fotoğrafla 46 pazaryerinde sat.',
    privacy: 'Gizlilik', terms: 'Şartlar', support: 'Destek',
    accessibility: 'Erişilebilirlik', deleteAccount: 'Hesabı Sil',
    rights: 'Tüm hakları saklıdır. Bağımsız satıcılar için yapıldı.',
  },
};

export default tr;
