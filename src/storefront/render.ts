...
export function renderStoreBody(
  ...
  // Üst yardımcı bar — sağa hizalı dil/kur seçicileri + hesap alanı
  ...
  html += `  <div class="sf-acct" id="pz-acct-root">\n`;
  html += `    <div id="pz-signin" class="sf-acct__signin" data-client-id="${escapeAttr(GOOGLE_CLIENT_ID)}">\n`;
  html += `      <button type="button" class="sf-acct__btn" id="pz-signin-btn" aria-label="${escapeHtml(mt(locale, 'signIn'))}">\n`;
  // Google "G" logosu — inline SVG
  html += '        <svg class="sf-acct__glogo" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" aria-hidden="true">\n';
  ...
  html += '        </svg>\n';
  html += `        <span class="sf-acct__btn-label">${escapeHtml(mt(locale, 'signIn'))}</span>\n`;
  html += '      </button>\n';
  html += '    </div>\n';
  // Kullanıcı avatar + açılır menü (auth.js doldurur; başlangıçta gizli)
  html += '    <div id="pz-user" class="sf-acct__user" hidden></div>\n';
  html += '  </div>\n'; // sf-acct
  html += '</div>\n';
...
export function renderProductBody(
  ...
  // Hesap alanı: auth.js tarafından doldurulur (product body'de pz-acct-root-pdp ile çakışma önlenir)
  html += `    <div class="sf-acct" id="pz-acct-root">\n`;
  html += `      <div id="pz-signin" class="sf-acct__signin" data-client-id="${escapeAttr(GOOGLE_CLIENT_ID)}">\n`;
  html += `        <button type="button" class="sf-acct__btn" id="pz-signin-btn" aria-label="${escapeHtml(mt(locale, 'signIn'))}">\n`;
  html += '          <svg class="sf-acct__glogo" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" aria-hidden="true">\n';
